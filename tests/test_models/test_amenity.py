...
class test_Amenity(test_basemodel):
    """Definess Test case for Amenity"""
    ...
    def test_name2(self):
        """Checks for name as string """
        new = self.value
        self.assertEqual(new, Amenity)
...
class test_Amenity_BaseModel(unittest.TestCase):
    ...
    def test_str_method(self):
        ...
        """
        inst = Amenity()
        str_o = "[Amenity] ({}) {}".format(inst.id, inst.__dict__)
        self.assertEqual(str_o, str(inst))
...
class TestAmenity(unittest.TestCase):
    ...
    def test_to_dict_creates_dict(self):
        ...
        print(am.__dict__)
        d_new = am.to_dict()
        self.assertEqual(type(d_new), dict)
        self.assertFalse("_sa_instance_state" in d_new)
        for attr in am.__dict__:
            if attr is not "_sa_instance_state":
                self.assertTrue(attr in d_new)
        self.assertTrue("__class__" in d_new)
    ...
    def test_to_dict_values(self):
        ...
        am = Amenity()
        d_new = am.to_dict()
        self.assertEqual(d_new["__class__"], "Amenity")
        self.assertEqual(type(d_new["created_at"]), str)
        self.assertEqual(type(d_new["updated_at"]), str)
        self.assertEqual(d_new["created_at"], am.created_at.strftime(t_format))
        self.assertEqual(d_new["updated_at"], am.updated_at.strftime(t_format))

    def test_str(self):
        """test that the str method has the correct output"""
        amenity = Amenity()
        str_x = "[Amenity] ({}) {}".format(amenity.id, amenity.__dict__)
        self.assertEqual(str_x, str(amenity))
